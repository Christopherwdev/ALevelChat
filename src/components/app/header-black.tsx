...
export default function Navigation({ isAuthenticated }: NavigationProps) {
  ...
  return (
    ...
    >
      <nav className="bg-[rgba(20,20,20,0.9)] backdrop-blur-[15px] border-b border-gray-800">
          ...
          <div className="flex items-center h-16 justify-between w-full">
            ...
            <div className="flex items-center min-w-[100px]">
              <Link href="/" className="flex items-center space-x-2">
                ...
                />
                <span className="text-xl font-bold text-white">AIToLearn</span>
              </Link>
            </div>
            {/* Exams center */}
            <div className="hidden lg:flex justify-center items-center space-x-0 py-2 px-3 bg-[rgba(255,255,255,0.05)] rounded-full">
              {exams.map((exam) => (
                <div
                  key={exam.name}
                  className="relative"
                ...
                >
                  <Link
                    ...
                    className={`px-4 py-2 text-md font-medium transition-colors rounded-md ${
                      pathname.startsWith(exam.path)
                        ...
                        : 'text-gray-100 hover:text-[#ff3b30] hover:bg-[rgba(255,59,48,0.10)]'
                    }`}
                  >
                    {exam.name}
                  </Link>
                  ...
                  {hoveredExam === exam.name && (
                    <div className="absolute left-0 mt-1 w-48 bg-[rgba(30,30,30,0.98)] rounded-xl shadow-lg py-2 z-20 border border-[#222] flex flex-col">
                      {exam.subjects.map((subject) => (
                        <Link
                          key={subject.name}
                          href={subject.path}
                          className="px-4 py-2 text-sm text-gray-100 font-semibold hover:bg-[rgba(255,59,48,0.10)] hover:text-[#ff3b30] rounded transition-colors"
                        >
                          {subject.name}
                        </Link>
                      ))}
            ...
            <div className="flex items-center space-x-4 min-w-[100px] justify-end">
              {isAuthenticated ? (
                <div className="relative">
                  ...
                  {showUserMenu && (
                    <div className="absolute right-0 mt-2 w-48 bg-[rgba(30,30,30,0.98)] rounded-lg shadow-lg py-1 z-10 border border-gray-800">
                      <Link
                        href="/profile"
                        className="block px-4 py-2 text-sm text-gray-100 hover:bg-[rgba(255,59,48,0.10)] transition-colors"
                      ...
                      <button
                        ...
                        className="block w-full text-left px-4 py-2 text-sm text-gray-100 hover:bg-[rgba(255,59,48,0.10)] disabled:opacity-50 transition-colors"
                      >
            ...
            <div className="flex lg:hidden">
              <button
                ...
                className="p-2 rounded-md text-gray-200 hover:bg-[rgba(255,255,255,0.08)]"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute left-0 right-0 bg-[rgba(30,30,30,0.98)] shadow-lg rounded-b-xl z-40 mt-0 px-4 py-2">
          {exams.map((exam) => (
            <div key={exam.name} className="mb-2">
              <button
                onClick={() => setExpandedExam(expandedExam === exam.name ? null : exam.name)}
                className="w-full text-left px-2 py-2 font-semibold text-gray-100 hover:text-[#ff3b30] flex justify-between items-center"
              >
                {exam.name}
                ...
                <div className="pl-4">
                  {exam.subjects.map((subject) => (
                    <Link
                      key={subject.name}
                      href={subject.path}
                      className="block px-2 py-1 text-gray-100 hover:text-[#ff3b30]"
