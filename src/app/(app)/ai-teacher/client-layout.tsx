...
export default function ClientLayout({ teachers, children }: ClientLayoutProps) {
  ...
  // Close sidebar when a teacher is selected
  useEffect(() => {
    if ((selectedTeacherId || selectedTeacher) && sidebarOpen) {
      setSidebarOpen(false);
    }
  }, [selectedTeacherId, selectedTeacher, sidebarOpen]);

  return (
    <div className="flex h-screen bg-gray-50">
      {/* Mobile hamburger menu - only shown when no teacher is selected */}
      ...
      {/* Mobile sidebar overlay - only shown when no teacher is selected */}
      {!selectedTeacherId && !selectedTeacher && sidebarOpen && (
        <div
      ...
      {/* Sidebar - only shown when no teacher is selected */}
      {!selectedTeacherId && !selectedTeacher && (
        <div className={`
