...
const App: React.FC = () => {
    ...
    const [pendingMarking, setPendingMarking] = useState(false);
    ...
    const generateContent = async () => {
        ...
        const prompt = `
            Generate a short IGCSE Chinese reading passage (around 200-300 characters) in **Traditional Chinese characters**.
            Then, generate at least 7 diverse reading comprehension questions based on the passage. The questions should be in English and cover the types found in IGCSE Chinese reading exams, such as:
            ...
            Provide the output as a single JSON object with the following structure. Note that you should not use the same questions as the sample below. Note that the questions below are samples for each question type, do not use same content of questions. Generate 7 questions that includes each question type. Generate questions from any topic about Chinese life.
            {
                "reading_test": {
    ...
    const handleSubmitAnswers = () => {
        collectUserAnswers();
        setPendingMarking(true);
    };

    useEffect(() => {
        if (pendingMarking) {
            markAnswersAI();
            setPendingMarking(false);
        }
    ...
    const renderContent = () => {
                            ...
                            <div key={result.question_id} className={`result-item ${result.correct ? 'correct' : 'incorrect'}`} style={{ marginBottom: '1rem' }}>
                                ...
                                <p><strong>Your Answer:</strong> {
                                    Array.isArray(result.user_answer)
                                        ...
                                        : typeof result.user_answer === 'object' && result.user_answer !== null
                                            ? (
                                                <ul style={{ margin: 0, paddingLeft: '1.25em' }}>
                                                    {Object.entries(result.user_answer).map(([k, v]) => (
                                                        <li key={k}><strong>{k.toUpperCase()}:</strong> {Array.isArray(v) ? v.join(', ') : String(v)}</li>
                                                    ))}
                                                </ul>
                                            )
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', textAlign: 'center', background: 'linear-gradient(to bottom, white, #ff3b3030' }} className='p-6'>
                ...
                <h2 className='text-5xl font-regular mb-4'>AI Reading Mock Test</h2>
                <p style={{ color: '#374151'}} className='mb-6'>Click 'Start Test' to generate a new reading passage and questions.</p>
