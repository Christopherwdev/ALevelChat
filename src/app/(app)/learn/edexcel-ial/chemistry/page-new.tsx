...
const App: React.FC = () => {
    ...
    const HomePageContent: React.FC = () => (
        <div className="rounded-2xl p-2 items-center flex justify-center ">
           <div className='max-w-4xl'>
            <div className="mt-10 flex flex-col md:flex-row items-stretch md:items-center gap-8 mb-6">
                ...
                <div className="flex flex-col flex-grow title-buttons-container">
                    ...
                    <p className="text-gray-700 text-lg max-w-3xl">Welcome to the Chemistry Revision Zone!<br></br>You can use the extensive resources below to prepare for your exams.</p>
                </div>
            </div>
    ...
    return (
        <React.Fragment>
            ...
            <style>
                ...
                .dark .header-nav-button:not(.active) { /* Changed from .home-subject-button:not(.active) */
            ...
            <div className="w-full h-screen flex flex-col">
                {/* Header */}
                <header className="flex flex-col md:flex-row justify-between items-center p-4 bg-white border-b border-[#00000020] dark:bg-gray-800 dark:border-gray-700 h-auto md:h-[80px] flex-wrap">
                    <div className="flex flex-col items-left mb-4 md:mb-0">
                        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">
                            <span id="subject-title">{CURRENT_SUBJECT}</span>
                        </h1>
                        <p className="text-xs text-gray-500 mt-1">Your comprehensive revision package</p>
                    </div>
                    
                    {/* Navigation Buttons */}
                    <nav className="flex flex-wrap justify-center md:justify-end gap-2">
                        <button
                            className={`header-nav-button ${activeHeaderSection === 'home' ? 'active' : ''}`}
                            onClick={() => setActiveHeaderSection('home')}
                        >
                            Home
                        </button>
                        <button
                            className={`header-nav-button ${activeHeaderSection === 'revision-notes' ? 'active' : ''}`}
                            onClick={() => setActiveHeaderSection('revision-notes')}
                        >
                            Revision Notes
                        </button>
                        <button
                            className={`header-nav-button ${activeHeaderSection === 'past-papers' ? 'active' : ''}`}
                            onClick={() => setActiveHeaderSection('past-papers')}
                        >
                            Past Papers
                        </button>
                        <button
                            className={`header-nav-button ${activeHeaderSection === 'battle-mode' ? 'active' : ''}`}
                            onClick={() => setActiveHeaderSection('battle-mode')}
                        >
                            Battle Mode
                        </button>
                        <button
                            className={`header-nav-button ${activeHeaderSection === 'tutor' ? 'active' : ''}`}
                            onClick={() => setActiveHeaderSection('tutor')}
                        >
                            Tutor
                        </button>
                    </nav>

                    {/* Editor Toolbar - kept for context, but not directly tied to new nav */}
