        ...
        try {
            ...
            tokens.forEach(token => {
                if (token.type === 'heading' && token.depth === 1) {
                    if (!currentNotesTitle) { // Only set if not already set by a previous render
                        setCurrentNotesTitle(token.text);
                    }
            ...
            if (!currentNotesTitle) {
                setCurrentNotesTitle(`${unitName}: ${CURRENT_SUBJECT} Notes`);
            }
    ...
    const ContinueLessonButton: React.FC = () => {
        ...
        const handleClick = () => {
            if (lastLesson) {
                ...
                // Optionally navigate to the first lesson if no last lesson exists
        ...
        if (lastLesson) {
            ...
            let sectionTitle = `Section ${lastLesson.sectionId.split('-').pop()}`;
            ...
            buttonContent = (
                <>
                    ...
                    <span className="text-sm text-gray-600 dark:text-gray-400 mt-1">Unit {lastLesson.unitIndex}: {sectionTitle}</span>
                </>
            );
        } else {
            buttonContent = (
        ...
        return (
            <div id="continue-lesson-button" className="continue-lesson-card" onClick={handleClick}>
                {buttonContent}
            </div>
        );
    };
        ...
        <React.Fragment>
            ...
            <style>
                ...
                .continue-lesson-card {
                    width: 200px; /* Fixed width as per image */
                    height: 200px; /* Fixed height to match title/buttons div */
            ...
            <div className="w-full h-screen flex flex-col">
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div id="left-sidebar" className="w-80 bg-white flex flex-col dark:bg-gray-800 dark:border-gray-700 overflow-y-auto p-4">
                        {/* Subject Home Button */}
                        <button
                            ...
                            <div id="unit-buttons-container" className="space-y-4">
                                {Array.from({ length: TOTAL_UNITS }, (_, i) => i + 1).map(unitIndex => {
                                    ...
                                    return (
                                        <div key={unitIndex}>
                                            <button
                                                className={`unit-button w-full text-left px-3 py-2 rounded-[15px] text-md font-medium flex items-center justify-between shadow-sm ${activeUnitIndex === unitIndex ? 'active' : ''}`}
                    ...
                    <div className="flex-1 overflow-y-auto p-6 bg-white dark:bg-gray-900 max-4xl">
                        ...
                        >
                            {activeUnitIndex === null ? (
                                // Home Page Content
                                <>
                                    <div className="flex flex-col md:flex-row items-stretch md:items-center gap-6 mb-[50px]">
                                        {/* <ContinueLessonButton /> */}
                                        
                                        <div className="flex flex-col flex-grow title-buttons-container">
                                        <nav className="text-gray-500 text-sm mb-0 font-semibold">
          ...
          <span>Edexcel IAL</span>
                                            ...
                                            <div className="flex items-end gap-2 mb-4 text-[60px] font-bold text-black" >
                                                <span className="">Edexcel IAL</span>
                                                <span style={{ color: 'var(--subject-primary-color)' }}>{CURRENT_SUBJECT}</span>
                                            </div>

                                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {revisionTools.map((tool) => (
            <div key={tool.id} className="bg-white p-4 hover:cursor-pointer rounded-4xl flex items-center space-x-4 border-[5px] border-[#00000010] transition duration-200 hover:border-[5px] hover:border-[#ff3b30] hover:ring-[10px] hover:ring-[#ff3b3020]">
              ...
              <div className="flex flex-col leading-tight">
                ...
                <span className="text-xl font-semibold text-gray-900">{tool.titleBottom}</span>
                                    ...
                                    <div id="lesson-pane" className="flex flex-row gap-6 mb-8 overflow-x-auto pb-4">
                                        {Array.from({ length: TOTAL_UNITS }, (_, i) => i + 1).map(unitIndex => (
                                            <UnitCard key={unitIndex} unitIndex={unitIndex} />
                                        ))}
                                    </div> */}
                                    ...
                                    <div className="bg-gray-100 rounded-xl p-2 shadow-inner mb-12 mt-8">
                                        {/* Horizontal Tab Bar */}
