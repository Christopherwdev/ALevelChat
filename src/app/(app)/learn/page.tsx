...
const LearnPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white p-0 font-inter">
      <div className="max-w-2xl mx-auto mt-5 p-4">
        <h1 className="text-3xl font-medium text-gray-800 text-left mb-5 font-bold">Subjects</h1>
        <div className="flex gap-3 mb-6">
          {exams.map((exam) => (
            <button
              key={exam.key}
              onClick={() => setSelectedExam(exam.key as keyof typeof subjectsByExam)}
              className={`px-5 py-2 rounded-full font-semibold border-2 transition-all duration-200 text-lg ${selectedExam === exam.key ? 'bg-black text-white border-black shadow' : 'bg-white text-black border-gray-300 hover:bg-gray-100'}`}
            >
              {exam.label}
            </button>
          ))}
        </div>
        <div className="bg-[#00000010] rounded-2xl border-box shadow-2xl shadow-[#00000010] p-0 pb-0 overflow-hidden border-2 border-[#00000020]">
